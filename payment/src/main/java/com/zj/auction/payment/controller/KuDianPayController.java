...
public class KuDianPayController {
    ...
    public ResponseData kuDianPay(@RequestBody Map<String, String> bodys) throws UnsupportedEncodingException {
        ...
        String res = userBillService.tradeCreate(bodys);
...
//            String passbackParams = (String) contentMap.get("body");
                String passbackParams = URLDecoder.decode(contentMap.get("body").toString(),"UTF-8");
    ...
    @ApiOperation("h5查询订单")
    @GetMapping(value = "/query/{transation_sn}")
    public ResponseData KuDianQuery(@PathVariable("transation_sn") String transationSn){
        String txOrder = userBillService.queryOrder(transationSn);
        JSONObject jsonObject = JSONObject.parseObject(txOrder);
        return ResponseData.ok(jsonObject);
    }

    @ApiOperation("h5关闭订单")
    @PostMapping(value = "/close/{transation_sn}")
    public ResponseData KuDianClose(@PathVariable("transation_sn") String transationSn){
       userBillService.closeOrder(transationSn);
