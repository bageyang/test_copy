...
//            String passbackParams = (String) contentMap.get("body");
    ...
    @ApiOperation("h5查询订单")
    @GetMapping(value = "/kudianQuery")
    public ResponseData KuDianQuery(@PathVariable String transationSn){
        String s = userBillService.queryOrder(transationSn);
        return ResponseData.ok(s);
    }

    @ApiOperation("h5关闭订单")
    @PostMapping(value = "/kudianQuery")
    public ResponseData KuDianClose(@PathVariable String transationSn){
