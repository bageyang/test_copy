...
//            throw new BaseException(SystemConstant.DATA_ILLEGALITY_CODE);
//        }
//        Optional<SysRoleTbl> oldRoleOpt = pcSysrolecfgRepository.findById(roleId);
//        SysRoleTbl oldRole = oldRoleOpt.orElseThrow(() -> throwException("未查询到角色信息"));
//        oldRole.setDeleteFlag(1);
//        oldRole.setUpdateTime(new Date());
//        oldRole.setAddUserid(user.getUserId());
//        pcSysrolecfgRepository.saveAndFlush(oldRole);
//
//        //要删除该角色下级的角色对应的权限
...
//    @Transactional(rollbackFor=Exception.class)
//    @Override
//    public Boolean saveRolePermission(SysRoleTbl sysRoleTbl, String menuStr) {
//        UserInfoTbl user =SecurityUtils.getPrincipal();
...
//            //	判断该用户与该角色的关系
//            Integer rolecount = pcSysUserRoleMdRepository.rolecount(user.getUserId(), sysRoleTbl.getRoleId());
//            if(rolecount>0) {
...
//     * @description: 删除下级角色
//     * @author: Mao Qi
//     * @date: 2019年8月5日下午8:35:03
//     * @param shCommonDaoImpl
//     * @param roleId
//     * @return: void
//     */
//    private static void delSubRole(SHCommonDaoImpl shCommonDaoImpl,Integer roleId) {
//        //根据角色查询下级角色id
//        String sql = "select role_id from sys_role_tbl where pid_str like CONCAT('%',:roleIdStr,'%') and delete_flag=0 ";
//        HashMap<String, Object> map = new HashMap<>(1);
//        map.put("roleIdStr", ","+roleId+",");
...
//            //删除下级角色所有对饮的权限
//            Integer roleid = PubFun.ObjectStrongToInt(f.get("role_id"));
//            String delSql = "update sys_role_tbl set delete_flag=1,update_time=now() where role_id=:roleId and delete_flag=0 ";
//            HashMap<String, Object> delMap = new HashMap<>(2);
//            delMap.put("roleId", roleid);
//            shCommonDaoImpl.updateJDBC(delSql, delMap);
//        });
